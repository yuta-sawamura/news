...
SHUSSEKIKUN(しゅっせきくん)は、管理者がユーザーの出席を管理できる Web システムです。<br/>
ユーザーは店舗に置かれた端末(PC、タブレット、スマートフォン)から、共有アカウントを用いて出席します。管理者はユーザー・出席等の情報を管理することができます。

## 作成の背景

私は以前、管理者として空手道場を運営していました。ユーザーの出席を紙で管理しており、アナログな管理方法で苦労した経験があります。そのような背景から管理者は、特別な機材を導入することなく端末を 1 台用意するだけで、出席管理ができる出席管理システムを作成しました。
...
-   管理者機能
    ...
    -   店舗管理機能
        -   登録 | 編集 | 削除
    -   ユーザー管理機能(1 つの店舗は複数のユーザーが属している)
        -   ユーザー情報確認機能(グラフ)
            -   累計ユーザー数 | 実働ユーザー数 | 入会者数 | 退会者数
        -   検索 | ページネーション | 登録 | 編集 | 退会 | 削除
    -   カテゴリー管理機能(１つのカテゴリーは複数のユーザーにひも付き、ユーザー検索の際に使用)
        -   登録 | 編集 | 削除
    -   お知らせ管理機能(店舗別にユーザー向けにお知らせ管理ができる)
        -   MarkDown 記法 | 検索 | ページネーション | 登録 | 編集 | 削除
    -   クラス管理機能
        -   登録 | 編集 | 削除
    -   スケジュール機能(クラスに曜日や時間を紐づけてスケジュールを作成)
        -   登録 | 編集 | 削除
-   ユーザー機能(ユーザーは店舗に置いてある端末から共有アカウントを使用する)
    -   検索機能
    -   出席機能
    -   出席ランキング閲覧機能
    -   お知らせ閲覧機能
